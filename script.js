...
function renderColors(colors) {
    ...
    for (let color of colors) {
        ...
        `
        const hexHtml = `
            <button class="hex-value" data-hex="${color.hex.value}" aria-label="Copy hex color ${color.hex.value} to clipboard">${color.hex.value}<span class="color-name">"${color.name.value || null}"</span></button>
        </div>
        `
